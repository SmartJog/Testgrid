...
class SelfTest(unittest.TestCase):
        ...
        def setUp(self):
            ...
            self.server = Server(host = "127.0.0.1", port = "3000", grid = self.grid)
            self.server.start()
            time.sleep(1)
        ...
        def test_basic_node_manipulation(self):
            data = self.request_get('http://127.0.0.1:3000/get_node?name=fake')
            self.assertIn("error", data)
            data = self.request_get('http://127.0.0.1:3000/get_node?name=node0')
            self.assertEqual(data, {'typename': 'fake node', 'hoststring': 'test@test', 'name': 'node0'})
            data = self.request_get('http://127.0.0.1:3000/is_available?name=node0')
            self.assertEqual(data, {"result": True})
            data = self.request_get('http://127.0.0.1:3000/is_allocated?name=node0')
            self.assertEqual(data, {"result": False})
            data = self.request_get('http://127.0.0.1:3000/is_quarantined?name=node0')
            self.assertEqual(data, {"result": False})
            data = self.request_get('http://127.0.0.1:3000/is_transient?name=node0')
            self.assertEqual(data, {"result": False})

        def test_add_remove_node(self):
            data = self.request_get('http://127.0.0.1:3000/get_nodes')
            ...
            data = self.request_get('http://127.0.0.1:3000/remove_node?name=node0')
            self.assertNotIn("error", data)
            data = self.request_get('http://127.0.0.1:3000/get_nodes')
            self.assertEqual(data, {'nodes': [{'typename': 'fake node', 'hoststring': 'test@test', 'name': 'node1'}]})
            self.request_post('http://127.0.0.1:3000/add_node', {"node_opt": {"name": "node", "type": "fake node"}, "username": "user"})
            data = self.request_get('http://127.0.0.1:3000/get_nodes')
            self.assertNotIn("error", data)
        ...
        def test_node_state(self):
            data = self.request_post('http://127.0.0.1:3000/quarantine_node', {"name": "node0", "reason": "test quarantine", "username": "user"})
            self.assertNotIn("error", data)
            data = self.request_get("http://127.0.0.1:3000/is_quarantined?name=node0&username=user")
            self.assertEqual(data, {"result": True})
            data = self.request_post('http://127.0.0.1:3000/rehabilitate_node', {"name": "node0", "username": "user"})
            self.assertNotIn("error", data)
            data = self.request_get("http://127.0.0.1:3000/is_quarantined?name=node0&username=user")
            self.assertEqual(data, {"result": False})

        def test_basic_session_manipulaion(self):
            data = self.request_get("http://127.0.0.1:3000/get_sessions")
            self.assertNotIn("error", data)
            self.assertEqual(data, {'sessions': [{'username': self.session.user.name, 'name':self.session.name}]})
            data = self.request_post("http://127.0.0.1:3000/close_session", {"session" :{"username":self.session.user.name, "name": self.session.name}})
            self.assertNotIn("error", data)
            data = self.request_get("http://127.0.0.1:3000/get_sessions")
            self.assertNotIn("error", data)
            self.assertEqual(data, {'sessions': []})
            data = self.request_post("http://127.0.0.1:3000/open_session", {"session": {"username": "test", "name": "test"}})
            self.assertNotIn("error", data)
            data = self.request_get('http://127.0.0.1:3000/get_sessions')
            self.assertNotIn("error", data)
        ...
        def test_session_allocate(self):
            data = self.request_get('http://127.0.0.1:3000/is_available?name=node0')
            self.assertEqual(data, {"result": True})
            data = self.request_post("http://127.0.0.1:3000/allocate_node", {"session" :{"username":self.session.user.name, "name": self.session.name}, "options": {}})
            self.assertNotIn("error", data)
            self.assertEqual(data, {'typename':'fake node','hoststring':'test@test', 'name': 'node0'})
            data = self.request_get('http://127.0.0.1:3000/is_available?name=node0')
            self.assertEqual(data, {"result": False})
            ...
            data = self.request_get('http://127.0.0.1:3000/is_allocated?name=node0')
            self.assertEqual(data, {"result": True})
            data = self.request_get('http://127.0.0.1:3000/session_contains?name=%s&username=%s&node=node0' % (self.session.name, self.session.user.name))
            self.assertEqual(data, {"result": True})
            data = self.request_get('http://127.0.0.1:3000/get_node_session?name=node0')
            ...
            self.assertEqual(data, {'nodes': [{'typename': 'fake node', 'hoststring': 'test@test', 'name': 'node0'}, {'typename': 'fake node', 'hoststring': 'test@test', 'name': 'node1'}]})
            data = self.request_post("http://127.0.0.1:3000/release_node", {"session" :{"username":self.session.user.name, "name": self.session.name}, "node": {"name": "node0"}})
            self.assertNotIn("error", data)
            data = self.request_post("http://127.0.0.1:3000/release_node", {"session" :{"username":self.session.user.name, "name": self.session.name}, "node": {"name":"node1"}})
            self.assertNotIn("error", data)
            data = self.request_get('http://127.0.0.1:3000/get_nodes_session?name=%s&username=%s' % (self.session.name, self.session.user.name))
